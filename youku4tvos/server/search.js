...
function updateSearch(doc, keyword) {
    if (keyword.length > 0) {
        let url = `${HOST}openapi-wireless/keywords/suggest?${IDS}&keywords=${keyword}`;
        console.log("url: "+url);
        ...
        let url = `${HOST}openapi-wireless/keywords/recommend?${IDS}`;
        console.log("url: "+url);
        getHTTP(url, function(datastr){
            var data = JSON.parse(datastr);
            var dom = doc.implementation;
...
function showSearchResults(text, pg, pz, callback) {
    var url = `${HOST}layout/smarttv/showsearch?copyright_status=1&video_type=1&keyword=${encodeURI(text)}&${IDS}`;
    var video_url = `${HOST}openapi-wireless/videos/search/${encodeURI(text)}?${IDS}&pg=${pg}&pz=${pz}`;
